...
export const BASE_32_CHARS = '0123456789ABCDEFGHIJKLMNOPQRSTUV';
...
export function base32ToHexadecimal(b: string): string {
    ...
    for (i = 0; i < b.length; i++) {
      const v = BASE_32_CHARS.indexOf(b.charAt(i).toUpperCase());
...
export function getTotp(secret: string, options: { epoch?: any; step?: any; digits?: any; algorithm?: any; }): string {
    ...
    const key = base32ToHexadecimal(secret);
    const opts = {
    ...
    const time = leftpad(decimalToHexadecimal(Math.floor(opts.epoch / opts.step)), 16, '0');
    const sha = new jsSHA(opts.algorithm, 'HEX');
    sha.setHMACKey(key, 'HEX');
    sha.update(time);
    const hmac = sha.getHMAC('HEX');
    const offset = hexadecimalToDecimal(hmac.substr(hmac.length - 1));
